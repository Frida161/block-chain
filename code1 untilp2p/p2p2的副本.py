...
class ServerThread(Thread):
    ...
    def run(self):
        ...
        signal = False
        self._sever.bind((self._ip_address, 1234))  # 绑定IP和端口
        self._sever.listen(512)  # 开始监听
        print(self._ip_address + " waits for connection")
        while True:
            try:
                ...
                if message[0:1] == b'\x11' :
                    self._my_queue.put("from" + address[0] + '：' + message[1:].decode('utf-8'))  # 发送给消息队列中
                elif message[0:1] == b'1':
                    receive_block(message[1:].decode('utf-8'))
                    print('Updated blockchain:', CTcoin.chain)
                elif message[0:1] == b'2':
                    receive_transaction(message[1:].decode('utf-8'))
                    self._my_queue.put('Please enter the number:\n1.mining\n2.transaction\n3.download previous block\n4.transmit previous block\n')
                    print('Updated transaction pool:', CTcoin.transactionPool)
                elif message[0:1] == b'3':
                    self._my_queue.put(message[1:].decode('utf-8'))
                    CTcoin.transactionPool = []
                elif message.decode('utf-8') == 'a':
                    self._my_queue.put('Someone requests to download previous block')
...
def main():
    global order, client_thread, is_start,ip_address,server_thread
    ...
    is_start = True
    ip_address = None
    print('orginal blockchain:', CTcoin.chain)
    print('orginal transactionPool:', CTcoin.transactionPool)
    while True:
        # 操作逻辑控制
        if is_start:
            order = input('Please enter the number:\n1.mining\n2.transaction\n3.download previous block\n4.transmit previous block\n')
            if order == '1':
                CTcoin.mineTransactionPool(get_host_ip())
                client_thread.send_message((address[0],5678), b'3' + bytes('someone has mined successfully','utf-8'))
            elif order == '2':
                ...
                ip = input('Please enter the ip address of other node:')
                port = 5678
                ip_address = ip, port
                is_start = False
            elif order == '4':
            ...
            if ip_address and message != '\r'  and order == '2':
                ...
                client_thread.send_message(ip_address, b'\x11' +  bytes(str(get_host_ip())+'transfer'+ money, 'utf-8'))
                ...
                message = input('quit()Exit the interface\n Successful transaction！\n')
                if message == 'quit()':
                    is_start = True
            elif order == '3':
                ...
                for index in range(1, len(CTcoin.chain)):
                            ...
                            client_thread.send_message((address[0],5678), b'1' + bytes(pack,'utf-8'))
                print('Transmit data....')
                message = input('quit()Exit the interface\n')
                if message == 'quit()':
                    is_start = True
